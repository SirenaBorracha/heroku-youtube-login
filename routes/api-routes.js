...
router.get('/comment/upload/:chanId/:parentId/:textOriginal', async(req, res) => {
    ...
    let result = await youtube.addComment(data.chanId, data.parentID, data.textOriginal, req.user.access_token, req.user.refresh_token, YID, YSEC)
    res.send(result)
})

router.post('/comments/uploader/', async(req, res) => {
    const data = req.body
    let result = await youtube.addComment(data.chanId, data.parentID, data.textOriginal, req.user._doc.access_token, req.user._doc.refresh_token, YID, YSEC)
    res.send(result)
})

router.post('/comments/reply/', async(req, res) => {
    const data = req.body
    let result = await youtube.replyToComment(data.commentId, data.chanId, data.parentID, data.textOriginal, req.user.access_token, req.user.refresh_token, YID, YSEC)
    res.send(result)
...
router.get('comments/reply/thread', async(req, res) => {
    ...
    let thread = await youtube.getReplies(commentId, API_KEY)
    res.json(thread)
})
